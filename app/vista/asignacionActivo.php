...
<div class="container-fluid">
            ...
            <form method="POST">
              <?php if (isset($asignaciones) && !empty($asignaciones)) {
                foreach ($asignaciones as $asignacion) { ?>
                  <tr>
                    ...
                    <td>
                      <button type="button" class="btn btn-success" data-bs-toggle="modal"
                        data-bs-target="#editar<?php echo $asignacion['id_asignacion'] ?>" name="editar">
                      ...
                      <button class="btn btn-danger btnEliminar" title="Eliminar" type="button"
                        value="<?php echo $asignacion['id_asignacion'] ?>" data-bs-toggle="modal"
                  ...
                  <div class="modal fade" id="editar<?php echo $asignacion['id_asignacion'] ?>" tabindex="-1"
                    aria-labelledby="editModalLabel<?php echo $asignacion['id_asignacion'] ?>" aria-hidden="true">
                    <div class="modal-dialog">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h1 class="modal-title fs-5" id="editModalLabel<?php echo $asignacion['id_asignacion'] ?>">Editar
                            Activo Asignado N°-<?php echo $asignacion['id_asignacion'] ?></h1>
                          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form method="POST" id="formEditarAsignacion" class="formEditarAsignacion">
                          <div class="modal-body d-flex flex-column gap-4">

                            <div>
                              <select class="form-select mb-3" name="id_activo" id="id_activo_edit"
                                aria-label="Default select example">
                                <option value="" selected disabled>Seleccione el Activo</option>
                                <?php
                            ...
                            <div>
                              <select class="form-select mb-3" name="cedula_empleado" id="cedula_empleado_edit"
                                aria-label="Default select example">
                            ...
                            <div>
                              <input class="form-control" type="text" name="Descripcion_Asignacion" id="descripcionEdit"
                                value="<?php echo htmlspecialchars($asignacion['Descripcion_Asignacion']) ?>" required>
                              <div class="invalid-feedback">Este Campo Solo Admite Letras, Por Favor Ingrese la
                                Descripcion
                                Nuevamente
                              </div>
                            </div>
                            <div>
                              <input class="form-control" type="date" id="fechaEdit" name="Fecha_asignacion"
                                value="<?php echo $asignacion['Fecha_asignacion'] ?>" required>
                              <div class="invalid-feedback">Por Favor Seleccione una Fecha Correcta</div>
                            </div>

                            <div class="modal-footer">
                              <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                              <button type="submit" class="btn btn-primary"
                                value="<?php echo $asignacion['id_asignacion'] ?>" name="editar">Editar Activo</button>
                            </div>
                          </div>
                        </form>
                      </div>
  ...
  <div class="modal fade" id="enviar_solicitud" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form method="POST" id="formCrearAsignacion" novalidate>
          <div class="modal-body d-flex flex-column gap-4">
            <div>
              <select class="form-select mb-3" name="id_activo" id="id_activo_create"
                aria-label="Default select example">
                <option value="" selected disabled>Seleccione el Activo</option>
                <?php
            ...
            <div>
              <select class="form-select mb-3" name="cedula_empleado" id="cedula_empleado_create"
                aria-label="Default select example">
            ...
            <div>
              <input class="form-control" type="text" id="descripcion" name="Descripcion_Asignacion"
                placeholder="Describe El estado del Activo" required>
              <div class="invalid-feedback">Por Favor Ingrese la Descripcion correctamente, Debe Tener Minimo 10
                Caracteres</div>
            </div>

            <div>
              <input class="form-control" type="date" name="Fecha_asignacion" id="fecha" placeholder="Fecha_asignacion"
                required>
              <div class="invalid-feedback">Por Favor Debe Ingrese la Fecha Actual</div>
            </div>
  ...
  <script type="module" src="Assets/js/validaciones/asignacionActivos.js"></script>